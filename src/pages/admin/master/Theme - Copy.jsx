...
const [processedFile, setProcessedFile] = useState(null);
  ...
  const handleView = async (s) => {
    ...
    if (!s?.pageNo) return [];
    const [start, end] = s?.pageNo?.split("-");
    const numStart = start ? Number(start) : 0;
    const numEnd = end ? Number(end) : numStart;

    const len = (numEnd || numStart) + 1 - numStart || 20;

    const pages = Array.from({ length: len }, (_, i) => Number(numStart) + i);

    try {
      const imgs = await Promise.allSettled(
        pages.map((p) => getPdfPageImage(pdfUrl, p)),
      );
      const formatedImg = imgs.map((item) => item?.value || "Not Available");

      setImages(formatedImg);
    } catch (err) {
      console.log("err >>>", err);
    }
  };

// const addPdfWatermark = async (file) => {
...
const addPdfWatermark = async (file) => {
  ...
  // ✅ Correct logo fetch
  const logoBytes = await fetch("/images/logo.png").then(res =>
    res.arrayBuffer()
  );
...
pages.forEach((page) => {
  ...
  // ✅ Compute page diagonal
  const diagonal = Math.sqrt(width * width + height * height);

  // ✅ Scale logo relative to diagonal
  const scaled = logoImage.scale(diagonal / logoImage.width * 0.6);
  // tweak 0.6 → controls watermark size
...
const handleFileChange = async (e) => {
  ...
  try {
    ...
    if (file.type === "application/pdf") {
      newFile = await addPdfWatermark(file);
    }

    // ✅ Store processed file OUTSIDE RHF
  ...
  return (
    <div className="container">
      ...
      <div className="listsec">
        ...
        <ul>
          {isLoadingThemes ? (
            <li>
              <div className="listbox academicterm text-center">Loading...</div>
            </li>
          ) : !themes?.length ? (
            <li>
      ...
      {activeModal && (
        ...
        >
          ...
          <div className="inputRow">
            ...
            {errors.theme?.message && <div className="invalid-feedback d-block">{errors.theme?.message}</div>}
          </div>
          ...
          <div className="inputRow">
            ...
            {errors.pageNo?.message && <div className="invalid-feedback d-block">
    ...
    />
                {errors.file?.message && <div className="invalid-feedback d-block">
                  {errors.file?.message}
                </div>}
              </div>
            )}
