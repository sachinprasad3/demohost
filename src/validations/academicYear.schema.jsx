...
export const academicYearSchema = yup.object({
  academicYear: yup
    ...
    .matches(/^\d{4}-\d{4}$/, "Format should be YYYY-YYYY")
    .test("is-unique", "Academic year already exists", function (value) {
      const { academicYears, originalValue } = this.options.context;
      if (!value || !academicYears) return true;
